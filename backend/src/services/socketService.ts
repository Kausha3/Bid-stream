...
class SocketService {
  ...
  initialize(httpServer: HttpServer): void {
    ...
    this.io.on('connection', (socket: TypedSocket) => {
      ...
      // Handle place bid
      socket.on('place_bid', (payload: PlaceBidPayload) => {
  ...
  private handleJoinAuction(socket: TypedSocket, auctionId: string): void {
  ...
  private handleLeaveAuction(socket: TypedSocket, auctionId: string): void {
  ...
  private async handlePlaceBid(
    ...
    if (result.previousLeaderPaymentIntentId) {
      ...
      console.log(
        `Cancelled payment hold for outbid user: ${result.previousLeaderPaymentIntentId}`
      );
    }
  ...
  private setupAuctionEndTimer(auctionId: string): void {
  ...
  private async handleAuctionEnd(auctionId: string): Promise<void> {
    ...
    if (result.status === 'SOLD' && auction.leadingPaymentIntentId) {
      ...
      const captureResult = await stripeService.capturePayment(
        auction.leadingPaymentIntentId
      );
      console.log(
        `Payment capture for winner ${result.winnerId}:`,
        captureResult
      );
    }
  ...
  emitToAuction(
    ...
    this.io?.to(`auction:${auctionId}`).emit(event, data as never);
  }
